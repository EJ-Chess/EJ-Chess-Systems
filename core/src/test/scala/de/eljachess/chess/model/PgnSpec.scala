...
class PgnSpec extends AnyFlatSpec with Matchers:
  ...
  it should "report only the first error when multiple headers are malformed" in {
    Pgn.decode("[BadHeader]\n[AlsoBad]\n\ne4 e5") match
      case Left(msg) => msg should include("Invalid PGN header")
      case Right(_)  => fail("Expected Left for malformed headers")
  }

  it should "propagate the first error past subsequent valid header lines" in {
    Pgn.decode("[BadHeader]\n[White \"Alice\"]\n[Black \"Bob\"]\n\ne4") match
      case Left(msg) => msg should include("Invalid PGN header")
